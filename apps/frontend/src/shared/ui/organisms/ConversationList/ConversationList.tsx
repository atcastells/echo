...
export const ConversationList = ({
  ...
  if (isLoading) {
    return (
      <div className={clsx('flex flex-col items-center py-8', className)}>
        ...
        <p className="mt-3 text-sm text-neutral-500">Loading conversations...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div
        className={clsx('flex flex-col items-center py-8 text-center', className)}
      >
        <div className="w-12 h-12 rounded-full bg-error-100 flex items-center justify-center mb-3">
          <Icon name="exclamation-circle" size="lg" className="text-error-500" />
        </div>
    ...
    return (
      ...
      >
        <div className="w-12 h-12 rounded-full bg-neutral-100 flex items-center justify-center mb-3">
          <Icon name="chat-bubble-left-right" size="lg" className="text-neutral-400" />
        </div>
        <p className="text-sm font-medium text-neutral-800 mb-1">
  ...
  return (
    <div
      ...
      aria-activedescendant={
        focusedIndex >= 0 ? `conversation-${conversations[focusedIndex].id}` : undefined
      }
    >
      {conversations.map((conversation, index) => (
        <div
          ...
          className={clsx(
            focusedIndex === index && 'ring-2 ring-primary-500 ring-inset rounded-lg'
          )}
        >
          <ConversationListItem
